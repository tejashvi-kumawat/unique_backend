...
export const getWhatsAppSenders = async (req, res) => {
  try {
    const db = getDatabase();
    const senders = await db.all('SELECT DISTINCT sender FROM whatsapp_context WHERE processed = TRUE');
    res.json({ senders: senders.map(s => s.sender) });
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch senders' });
  }
};

export const saveSenderMapping = async (req, res) => {
  try {
    const { user, partner } = req.body;
    if (!user || !partner) {
      return res.status(400).json({ error: 'Both user and partner names are required' });
    }
    const db = getDatabase();
    await db.run('DELETE FROM sender_mapping');
    await db.run('INSERT INTO sender_mapping (role, sender_name) VALUES (?, ?)', ['user', user]);
    await db.run('INSERT INTO sender_mapping (role, sender_name) VALUES (?, ?)', ['partner', partner]);
    res.json({ success: true });
  } catch (error) {
    res.status(500).json({ error: 'Failed to save sender mapping' });
  }
};
